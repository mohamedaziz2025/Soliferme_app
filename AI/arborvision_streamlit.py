...
# ---------------- Analysis page ----------------
    ...
    # allow the user to enter manual values which will be used as an override.
    ...
    # Determine whether sensors returned usable values
    sensors_ok = bool(sensors and sensors.get('ultrasound_m') not in (None, 0) and sensors.get('laser_m') not in (None, 0))

    if sensors_ok:
        u_val = sensors.get('ultrasound_m')
        l_val = sensors.get('laser_m')
        st.write(f"Capteurs — Ultrason: {u_val} m  |  Laser: {l_val} m")
    else:
        st.write("Capteurs non disponibles ou lecture partielle — estimation par pixels utilisée")
        ...
        with col_l:
            manual_l = st.number_input('Laser (m) - valeur manuelle', min_value=0.0, value=(st.session_state.get('manual_sensors') or {}).get('laser_m') or 0.0, step=0.01, format="%.2f")
        ...
        if st.button('Utiliser ces valeurs manuelles'):
            ...
            l_store = manual_l if manual_l > 0 else None
            st.session_state['manual_sensors'] = {'ultrasound_m': u_store, 'laser_m': l_store}
...
# store analysis basic info
    ...
    if st.button('🔬 Lancer l\'analyse complète', use_container_width=True):
        ...
        sensors_run = st.session_state.get('manual_sensors') or sensors or simulate_sensors()
