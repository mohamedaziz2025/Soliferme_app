...
# ----------- Calcul de hauteur RÈGLE DE TROIS AVEC CLASSIFICATION DE TAILLE -----------
base_pos, top_pos = detect_tree_extremes(tree_mask)

print(f"\n🔢 CALCUL AVEC CLASSIFICATION DE TAILLE:")
...
# CLASSIFICATION DE LA TAILLE DE L'ARBRE
def classify_tree_size(pixel_height, image_height):
    """Classifier l'arbre selon sa taille relative dans l'image"""
    coverage = pixel_height / image_height
    
    if coverage < 0.3:  # Petit arbre (moins de 30% de l'image)
        return "PETIT", coverage
    elif coverage < 0.6:  # Arbre moyen (30-60% de l'image)
        return "MOYEN", coverage
    else:  # Grand arbre (plus de 60% de l'image)
        return "GRAND", coverage

tree_size, coverage = classify_tree_size(tree_pixel_height, height)
print(f"   Classification: {tree_size} arbre ({coverage:.1%} de l'image)")

# DONNÉES DE RÉFÉRENCE ADAPTÉES SELON LA TAILLE
if tree_size == "PETIT":
    # Pour petits arbres (comme votre référence)
    REF_PIXELS = 2185     # Votre mesure exacte
    REF_HEIGHT_M = 1.35   # Hauteur réelle
    SIZE_FACTOR = 1.0     # Facteur neutre
    print("   → Utilisation référence petits arbres")
    
elif tree_size == "MOYEN":
    # Pour arbres moyens (ajustement empirique)
    REF_PIXELS = 2185 * 1.5  # Plus grand que la référence
    REF_HEIGHT_M = 1.35 * 1.8  # Plus haut proportionnellement
    SIZE_FACTOR = 1.1     # Légère correction
    print("   → Utilisation référence arbres moyens")
    
else:  # GRAND
    # Pour grands arbres (ajustement empirique)
    REF_PIXELS = 2185 * 2.5  # Beaucoup plus grand
    REF_HEIGHT_M = 1.35 * 3.0  # Plus haut mais avec saturation
    # Ajustement du facteur de taille selon la hauteur en pixels
    if tree_pixel_height < 1000:
        SIZE_FACTOR = 4.0  # Correction pour petits arbres grands
    else:
        SIZE_FACTOR = 0.9  # Correction pour éviter surestimation
    print("   → Utilisation référence grands arbres")

print(f"   Référence adaptée: {REF_PIXELS:.0f} pixels = {REF_HEIGHT_M:.2f}m")

# RÈGLE DE TROIS AVEC CORRECTION DE TAILLE
ratio = tree_pixel_height / REF_PIXELS
H_m_base = (tree_pixel_height / REF_PIXELS) * REF_HEIGHT_M
H_m = H_m_base * SIZE_FACTOR

print(f"   Ratio: {tree_pixel_height}/{REF_PIXELS:.0f} = {ratio:.3f}")
print(f"   Hauteur de base: {H_m_base:.3f}m")
print(f"   Facteur taille: {SIZE_FACTOR}")
print(f"   Hauteur finale: {H_m:.3f}m")

# Arrondir à 2 décimales
H_m = round(H_m, 2)

# INCERTITUDE ADAPTÉE SELON LA TAILLE
if tree_size == "PETIT":
    uncertainty = 0.15  # ±15% (plus précis car proche de la référence)
elif tree_size == "MOYEN":
    uncertainty = 0.25  # ±25% (moyennement précis)
else:  # GRAND
    uncertainty = 0.35  # ±35% (moins précis car extrapolation)

# Vérification si l'arbre est coupé par les bords de l'image
if tree_pixel_height > height * 0.9:
    print("   ⚠️ Attention: L'arbre semble coupé par les bords de l'image.")
    print("   L'estimation de hauteur peut être imprécise.")
    uncertainty *= 1.5  # Augmenter l'incertitude

# DBH ADAPTÉ SELON LA TAILLE
if tree_size == "PETIT":
    DBH_RATIO = 0.12  # 12% pour petits arbres
elif tree_size == "MOYEN":
    DBH_RATIO = 0.10  # 10% pour arbres moyens
else:  # GRAND
    DBH_RATIO = 0.08  # 8% pour grands arbres (proportionnellement plus fin)

DBH_m = round(H_m * DBH_RATIO, 2)

print(f"   DBH ratio ({tree_size}): {DBH_RATIO}")
print(f"   DBH estimé: {DBH_m}m")
print(f"   Incertitude ({tree_size}): ±{uncertainty*100:.0f}%")
...
cv2.putText(img_out, f"Taille: {tree_size} ({coverage:.1%})", 
...
print("\n" + "="*50)
print("RÉSULTATS ESTIMATION ARBRE - CLASSIFICATION TAILLE")
print("="*50)
print(f"Image analysée: {os.path.basename(img_path)}")
print(f"Classification: {tree_size} arbre ({coverage:.1%} de l'image)")
print(f"Hauteur mesurée: {tree_pixel_height} pixels")
print(f"Référence utilisée: {REF_PIXELS:.0f}px = {REF_HEIGHT_M:.2f}m")
print(f"Ratio: {tree_pixel_height}/{REF_PIXELS:.0f} = {ratio:.3f}")
print(f"Facteur taille: {SIZE_FACTOR}")
print(f"Hauteur estimée: {H_m} m ±{uncertainty*100:.0f}%")
print(f"Diamètre DBH: {DBH_m} m (ratio {DBH_RATIO})")
